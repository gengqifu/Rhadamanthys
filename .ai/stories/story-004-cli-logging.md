...
## Tasks
1. 测试任务
   1.1 - [ ] 设计测试：参数解析（log-interval-ms/format/include/exclude）、预检失败场景（版本/依赖/libclang/路径）
   1.2 - [ ] 编写测试断言：退出码 0/1/2/3、中文错误输出、日志频率配置
   1.3 - [ ] 设计测试：启动时规则库更新流程的提示（开始/进度/完成），含有更新/无更新/失败场景
2. 开发任务
