...
## Tasks
1. 测试任务
   - [x] 设计测试夹具：Swift/ObjC 样例覆盖 ATT 缺失+跟踪 SDK、外链/第三方支付、缺苹果登录、私有 API 反射、明文 HTTP、后台模式缺失
   - [x] 编写测试断言：命中规则ID、风险级、高置信/需复核、中文建议、证据行号
2. 开发任务
   - [x] 集成 libclang/SourceKitten 输出解析，遍历 Swift/ObjC（支持过滤）
   - [x] 实现检测：ATT/广告 SDK/IDFA、支付、登录、私有 API/反射、HTTP、后台模式
   - [x] 应用过滤（目录/大小）、返回 Findings（证据/中文建议/风险/需复核）
3. 验证
   - [x] 运行测试，全部通过后标记完成
