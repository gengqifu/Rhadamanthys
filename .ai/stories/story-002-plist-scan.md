...
## Tasks
1. 测试任务
   - [x] 设计测试夹具：多语言/缺失权限文案、ATS 全关/例外过多、异常后台模式、缺 Sign in with Apple/URL Schemes
   - [x] 编写测试断言：规则命中、风险级与中文建议、需复核标记
2. 开发任务
   - [x] 解析 Info.plist/Entitlements（含多语言），实现检测逻辑
   - [x] 产出 Findings（文件/行/片段、中文理由与建议、风险级、需人工复核标记）
3. 验证
   - [x] 运行测试，全部通过后标记完成
